...
from longterm_tracking import LongTermSiamFC
...
def evaluate_tracker(dataset_path, network_path, results_dir, visualize):
    ...
    tracker = LongTermSiamFC(net_path=network_path)
    ...
    for sequence_name in sequences:
        ...
        for i in range(1, sequence.length()):
            ...
            if visualize:
                ...
                if not tracker.target_visible:
                    for i in range(tracker.nr_samples):
                        cv2.circle(img, (int(tracker.samples[0][i]), int(tracker.samples[1][i])), 3, (0, 255, 0))
                cv2.imshow('win', img)
                key_ = cv2.waitKey(10)
                if key_ == 27:
                    exit(0)
